...
import ueccv.distances as dist

import ueccv.sparse as sparse

from ueccv.utils import (
    tau_rand_int,
    deheap_sort,
    submatrix,
    ts,
...
from ueccv.rp_tree import rptree_leaf_array, make_forest
from ueccv.nndescent import (
    make_nn_descent,
    make_initialisations,
    make_initialized_nnd_search,
    initialise_search,
)
from ueccv.spectral import spectral_layout
...
#GT = pd.read_csv("C:/Users/GOOD DAY/PycharmProjects/Datasets and AE whieghts/datasets/USPS/usps_y.csv")
#GT= pd.read_csv("datasets/CIFAR10/cifar10_y.csv")
#GT= pd.read_csv("datasets/STL/stl10_y.csv")
#GT=np.array(GT)
#GT=GT[1:]

#@numba.njit(fastmath=True, nopython=False)
def clustering_layout_SGD(embedding, centroids, initial_alpha):
    #initial_alpha=1.0
    alpha=initial_alpha
    beta_1 = 0.9
    dim=embedding.shape[1]
    n_clusters=centroids.shape[0]
    q=S(embedding, centroids)
    p=T(q)
    for i in range(embedding.shape[0]):
...
def clustering_layout_adam(embedding, centroids):
    ...
    for k in range(n_clusters):
        ...
        for d in range(dim):
            grad_d = clip(grad_coeff * (x[d] - centroid[d]))
...
def CE_gradient(Y, a, b):
    """
    Compute the gradient of Cross-Entropy (CE)
    """
    y_diff = np.expand_dims(Y, 1) - np.expand_dims(Y, 0)
    inv_dist = np.power(1 + a * np.square(euclidean_distances(Y, Y))**b, -1)
    Q = np.power(0.001 + np.square(euclidean_distances(Y, Y)), -1)
    np.fill_diagonal(Q, 0)
    Q = Q / np.sum(Q, axis = 1, keepdims = True)
    fact=np.expand_dims(a*(1e-8 + np.square(euclidean_distances(Y, Y)))**(b-1) - Q, 2)
...
def optimize_layout(
    ...
    for n in range(n_epochs):
        ...
        for i in range(epochs_per_sample.shape[0]):
            if epoch_of_next_sample[i] <= n:
                ...
                for k in range(n_clusters):
                    ...
                    if dist_squared > 0.0:
                        #grad_coeff = (-2.0 * P[j, k] * a * b* pow(dist_squared, b - 1.0))
                        #grad_coeff /= ( a * pow(dist_squared, b) + 1.0)
                        grad_coeff = 2.0 * pow((a * pow(dist_squared, b) + 1.0), -1.0)
                        grad_coeff *= (target[j, k] - soft[j, k])
                    else:
                        grad_coeff = 0.0

                    for d in range(dim):
                        grad_d = clip(grad_coeff* (current[d] - centroid[d]))
                        current[d] += grad_d * alphac

                    if dist_squared > 0.0:
                        #grad_coeff = (-0.1 * P[j, k] * a * b * pow(dist_squared, b - 1.0))
                        #grad_coeff /= (a * pow(dist_squared, b) + 1.0)
                        grad_coeff = 0.001 * pow((a * pow(dist_squared, b) + 1.0), -1.0)
                        grad_coeff *= (target[j, k] - soft[j, k])
                    else:
                        grad_coeff = 0.0
        ...
        #y_pred = Q(head_embedding, centroids, a, b)
        y_pred = q.argmax(1)
        ...
        if verbose and n % int(n_epochs / 10) == 0:
            print(
                "\tcompleted ", n, " / ", n_epochs, "epochs", "Lr= ", alphac
            )
...
#d = pd.read_csv("C:/Users/GOOD DAY/PycharmProjects/umap_clustering/umap/datasets/mnist_ae_DEC.csv")
d = pd.read_csv("C:/Users/GOOD DAY/PycharmProjects/umap_clustering/umap/datasets/nltk_reuters_ae_DEC.csv")
#d = pd.read_csv("datasets/usps_ae_DEC.csv")
